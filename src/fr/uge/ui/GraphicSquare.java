...
// private final Rectangle2D.Float rectangle = new Rectangle2D.Float(0, 0, 0,
// 0);

// void draw(ApplicationContext context, float x, float y, int size, Color
// color) {
// context.renderFrame(graphics -> {
// // hide the previous rectangle
...
// private static final Color COLOR_FOREST = Color.GREEN; // new Color(46, 111,
// 64); // 39, 66, 26
// private static final Color COLOR_WETLAND = new Color(166, 185, 111);
// private static final Color COLOR_MOUNTAIN = Color.GRAY; // new Color(105,
// 105, 105);
...
// private int heightShift = 0; /* that's count when user wants to move his
// screen into left / right */
// private int widthShift = 0; /* that's count when user wants to move his
// screen into up / down */
...
// // this.spaceBetweenTileAndToken = 10; /* to determine later based on width
// and
// // height */
// // this.spaceBetweenCouples = 10; /* to determine later based on width and
// // height */
// // this.sizeOfScoringCard = 10; /* to determine later based on width and
// height
// // */
// // this.sizeOfPlayerBoard = 10; /* optional */
// }

// private static void drawSquare(ApplicationContext context, float x, float y,
...
// // var rectangle = new RoundRectangle2D.Float(x, y, width, height, height /
// 10,
// // height / 10);
...
// private static void drawCircle(ApplicationContext context, float x, float y,
// int size, Color color) {
...
// public static void drawLine(ApplicationContext context, int x1, int y1, int
// x2, int y2, Color color) {
// context.renderFrame(graphics -> {
// graphics.setColor(color);
...
// // ApplicationContext context, WildlifeType token, int tokenSize, int x, int
// y,
// // ImageLoader images) {
...
// ApplicationContext context, Player player, int width, int height, int
// figureSize) {
// var listCells = player.getEnvironment().getCells();
// // drawSquare(context, (int) (figureSize / 2), (int) (figureSize / 2),
// // figureSize, Color.BLACK);
// // drawSquare(context, (int) ((width / 2) + figureSize / 2), (int) ((height /
// 2)
...
// var xAnimal = (int) ((x - (figureSize / 2) + figureSize / 20) + placement *
// (figureSize / 2.5));
...
// case FOREST -> drawRectangle(context, baseXOffset, yPosition, figureSize,
// figureSize, COLOR_FOREST);
// case WETLAND -> drawRectangle(context, baseXOffset, yPosition, figureSize,
// figureSize, COLOR_WETLAND);
// case MOUNTAIN -> drawRectangle(context, baseXOffset, yPosition, figureSize,
// figureSize, COLOR_MOUNTAIN);
// case RIVER -> drawRectangle(context, baseXOffset, yPosition, figureSize,
// figureSize, COLOR_RIVER);
// case PRAIRIE -> drawRectangle(context, baseXOffset, yPosition, figureSize,
...
// switch (token) {
// case BEAR ->
// drawImage(graphics, images.image(token.ordinal()), circleX, circleY,
...
// int x, int y, int width, int height, int figureSize, boolean
// isFamilyScoringCard) {
// /*
...
// private static void showPossibleCoordinates(ApplicationContext context,
...
// if (cell == null) {
// throw new IllegalArgumentException("cell is null in
// showPossibleCoordinates()");
// }
// if (!cell.isOccupiedByTile()) {
...
// System.err.println("Coordinates are invalid 1"); /* for tests, to delete
// later */
// return false;
// }
...
// game.board().setDefaultTokensAreUpdated(); /* that means, next person can
...
// int choice;
// do {
// choice = Integer.parseInt(IO.readln("Please choose ONLY from 1 to 4 to take a
// couple: (Tile, Token)\n"));
...
// int width, int height, int figureSize, int tokenSize) {
// Objects.requireNonNull(player, "player is null in
// showPossibleTokenPlacement()");
// Objects.requireNonNull(token, "token is null in
// showPossibleTokenPlacement()");
...
// showPossibleTokenPlacement(context, player, chosenToken, width, height,
// figureSize, tokenSize);

// var userCoordinates = getCoordinatesFromUser(context);
// if (userCoordinates == null) {
// System.err.println("Coordinates are invalid WHEN PLACING TOKEN"); /* for
// tests, to delete later */
// return true;
// }
// System.err.println("Coordinates (TokenPlacement) : (" + userCoordinates.x() +
// "," + userCoordinates.y() + ")");
...
// var tokenWasPlaced = player.getEnvironment().placeAnimal(currCell,
// chosenToken);
// /* for tests, to delete later */
// System.err.println(tokenWasPlaced ? "Token was placed successfully" : "Token
// wasn't placed");
...
// var userCoordinates = getCoordinatesFromUser(context);
// if (userCoordinates == null) {
// System.err.println("Coordinates are invalid WHEN PLACING TILE"); /* for
// tests, to delete later */
// return false;
// }
// var coordinatesForMap = new Coordinates(
...
// if (possibleCoordinates.stream().anyMatch(coordinates ->
// coordinates.equals(coordinatesForMap))) {
// var currCell = player.getEnvironment().getCell(coordinatesForMap);
...
// // ApplicationContext context, Game game, int width, int height, int
// figureSize,
// // int tokenSize) {
// // Objects.requireNonNull(game);
// // while (!game.turnManager().isGameEnd()) {
// // drawRectangle(context, 0, 0, width, height, Color.WHITE); /* clear the
// screen
// // */
...
// // // System.out.println("We have two players, please introduce
// yourselves.\n");
...
// private static void drawString(ApplicationContext context, String text, int
// x, int y, int fontSize, Color color) {
// context.renderFrame(graphics -> {
// graphics.setColor(color);
// //
...
// private static boolean insideRectangle(int x, int y, int x1, int y1, int x2,
// int y2) {
// return x >= x1 && x <= x2 &&
// y >= y1 && y <= y2;
// }
...
// ApplicationContext context, int x, int y, int figureSize, int
...
// drawString(context, i - (centerX / figureSize) + ", " + (j - (centerY /
// figureSize)),
// i * figureSize + (figureSize / 3),
// j * figureSize + (figureSize / 2),
// 15, Color.BLACK);
...
// ApplicationContext context, int width, int height, int
// environmentSquareWidth, int environmentSquareHeight) {
// drawRoundRectangle(context,
...
// ApplicationContext context, int width, int figureSize, int
// environmentSquareHeight) {
// drawRoundRectangle(context,
// width - (int) (figureSize * 2.35) - (int) (figureSize / 3), // width
// figureSize, // height
...
// int width, int environmentSquareWidth, int environmentSquareHeight, Color
// color) {
// drawRoundRectangle(context,
...
// // drawRoundRectangle(context, x1, y1, x2, y2, figureSize / 10, figureSize /
// 10,
// // Color.PINK);
// // }
...
// Game game = new Game(board, turnManager, listOfPlayers,
// Constants.VERSION_SQUARE);
// // gameLoopVersionSquare(context, game, width, height, figureSize,
// tokenSize);
// // calculateAndShowScore(game, familyOrIntermediate);
...
// // environmentSquareHeight / 10,
// // Color.RED);
// // drawString(context, "Welcome to the Cascadia game!", width / 2 - 150,
// height / 2, 20, Color.WHITE);
...
// // width - (int) (figureSize * 2.35) - (int) (figureSize / 3), figureSize, //
// x,
// // y
...
// // (int) (environmentSquareWidth / 5),
// // (figureSize * 1) + (int)(figureSize * 0.8), // (int)
// (environmentSquareHeight
// // / 3),
// // Color.BLUE);
...
// // (int) (environmentSquareWidth / 5),
// // (figureSize * 1) + (int)(figureSize * 0.9), // (int)
// (environmentSquareHeight
// // / 3),
// // Color.YELLOW);
...
// // (int) (environmentSquareWidth / 5),
// // (figureSize * 1) + (int)(figureSize * 1.2), // (int)
// (environmentSquareHeight
// // / 3),
// // Color.GREEN);
...
// restartEnvironmentRectangle(context, width, height, environmentSquareWidth,
// environmentSquareHeight);
// restartGameBoardRectangle(context, figureSize, environmentSquareHeight);
// restartScoringCardRectangle(context, width, figureSize,
// environmentSquareHeight);
...
// if (handleTokenChange(context, game, figureSize, (int) (width *
// coeffFontSizeInstructions))) {
// System.err.println("Changing Tokens");
// restartRectangleWithText(context,
// "Changing tokens Tokens", (int) (width - (environmentSquareWidth / 3.5)),
// figureSize / 2,
// (int) (width * coeffFontSizeInstructions),
// width, environmentSquareWidth, environmentSquareHeight, Color.BLACK);
// }
...
// // sleep(1000);
// // if (true){
// // System.err.println("Coordinates are invalid"); /* for tests, to delete
// later
// // */
// // context.dispose();
// // return;
...
// // System.err.println("Changing Tokens");
// restartRectangleWithText(context,
// "Place a tile", (int) (width - (environmentSquareWidth / 3.5)), figureSize /
// 2,
// (int) (width * coeffFontSizeInstructions),
// width, environmentSquareWidth, environmentSquareHeight, Color.BLACK);
...
// // boolean flag;
// // do {
// // flag = handleTilePlacement(context, currentPlayer, chosenTile, width,
// height,
// // figureSize);
// // } while (!flag);
// showPossibleCoordinates(context, currentPlayer, width, height, figureSize);

// handleTilePlacement(context, currentPlayer, chosenTile, width, height,
// figureSize);
...
// restartRectangleWithText(context,
// "Place " + chosenToken + " token", (int) (width - (environmentSquareWidth /
// 3.5)), figureSize / 2,
// (int) (width * coeffFontSizeInstructions),
// width, environmentSquareWidth, environmentSquareHeight, Color.BLACK);

// handleTokenPlacement(context, currentPlayer, chosenToken, width, height,
// figureSize, tokenSize);
...
// // area.draw(context, location.x(), location.y(), figureSize / 10,
// Color.BLACK);
// }
